...
Toda a aplicação é baseada em um comando chamando `notas-musicais`. Esse comando tem um subcomando relacionado a cada ação que a aplicação pode realizar. Como `escala`, `acorde`, `campo-harmonico`
...
poetry run notas-musicais --help
 ...
 Usage: notas-musicais.cmd [OPTIONS] COMMAND [ARGS]...
...
As informações sobre os subcomandos podem ser accessadas usando a flag `--help` após o nome do parâmetro. Um exemplo do uso do `--help` nos campos harmônicos:

```bash
poetry run notas-musicais campo-harmonico --help

 Usage: notas-musicais.cmd campo-harmonico [OPTIONS] [TONICA] [TONALIDADE]
